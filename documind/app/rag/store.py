"""Embedded Qdrant vector store with per-user isolation."""
import uuid
...
def add_chunks(user_id: int, doc_id: str, filename: str, chunks: list[dict], vectors: list[list[float]]):
    pts = []
...
def search(user_id: int, query_vec: list[float], top_k: int, doc_id: str | None = None):
